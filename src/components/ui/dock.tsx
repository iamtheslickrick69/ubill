...
function DockIcon({ icon: Icon, label, badge }: { icon: any; label: string; badge?: string }) {
  return (
    <button
      className="hover-halo group relative grid h-12 w-12 place-items-center rounded-xl ring-1 ring-white/10 bg-gradient-to-b from-neutral-800/60 to-neutral-900/70 backdrop-blur-xl shadow-lg transition-all duration-300 hover:-translate-y-1 hover:scale-[1.05] sm:h-14 sm:w-14"
      aria-label={label}
    >
      <Icon className="h-5 w-5 text-white transition-all duration-300 group-hover:scale-110 group-hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.6)]" strokeWidth={1.5} />
      {badge ? (
        <span className="absolute -right-2 -top-2 grid h-5 w-5 place-items-center rounded-full bg-white text-[10px] font-semibold text-neutral-900 ring-1 ring-white/80 sm:h-5 sm:w-5 sm:text-[10px] drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]">
          {badge}
        </span>
      ) : null}
      <span className="tooltip pointer-events-none absolute -bottom-6 translate-y-1/2 text-[9px] tracking-wide text-white/70 sm:text-[10px] drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]">
        {label}
      </span>
    </button>
  );
